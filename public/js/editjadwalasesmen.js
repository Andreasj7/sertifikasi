...
const buildJadwalAsesiField = async (id) => {
    ...
    const jadwalAssesmen = document.getElementById('pengisian_jadwal_asesmen');
    ...
    jadwalAssesmen.value = jadwalAsesi.schedule;
    pengerjaan.checked = jadwalAsesi.isProcessed;
};
...
const editJadwalAsesi = async (id) => {
    ...
    submitBtn.addEventListener('click', async (event) => {
        ...
        if (responseJson.status === 'success') {
            window.location.href = '#datajadwalasesmen';
        } else {
            window.location.href = '#datajadwalasesmen';
        }
    });
