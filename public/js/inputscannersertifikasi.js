...
const getScannerSertifikasi = async (id) => {
    const accessToken = localStorage.getItem('accessToken');
    const response = await fetch(`/api/cert-scanners/${id}`, {
        method: 'GET',
        headers: {
    ...
    console.log(id)
    return responseJson.data.certScanner;

};

const buildScannerSertifikasi = async (id) => {
    const certScanner = await getScannerSertifikasi(id);
    ...
    console.log(certScanner)
    const { printBlank } = certScanner;
    ...
    scanner.value = certScanner.date;
    berhasil.checked = certScanner.isProcessed;
};
...
const editScanner = async (id) => {
    ...
    submitBtn.addEventListener('click', async (event) => {
        ...
        const data = {
            date: document.getElementById('scanner').value,
        ...
        if (responseJson.status === 'success') {
            window.location.href = '#scannersertifikasi';
        } else {
...
await buildScannerSertifikasi(id);
