...
const getCetakFoto = async () => {
    const accessToken = localStorage.getItem('accessToken');
    const response = await fetch('/api/print-assesions?isProcessed=true', {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${accessToken}`,
    ...
    const responseJson = await response.json();
    const printAssesions = responseJson.data.printAssesions;

    return printAssesions;
};

const buildTable = () => {
    getCetakFoto().then((printAssesions) => {
        const dataTable = $('#dataTable').DataTable();
        printAssesions.forEach((printAssesion) => {
            const tukName =
                printAssesion?.assTestResult?.baSk?.assesmentSchedule?.assesion
            ...
            const cetakProcessed = printAssesion?.isProcessed || false;
            ...
            const data = {
                ...
                Action: `
                    <a
                        href="#inputcetakfoto?id=${printAssesion.id || ''}"
        ...
        $('#dataTable tbody').on('click', 'tr', async function () {
            ...
            const id =
                printAssesions[rowData].assTestResult.baSk.assesmentSchedule
