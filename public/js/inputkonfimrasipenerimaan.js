...
const getTuk = async (id) => {
    const accessToken = localStorage.getItem('accessToken');
    const response = await fetch(`/api/tuk-confirmations/${id}`, {
        method: 'GET',
        headers: {
    ...
    console.log(id);
    return responseJson.data.tukConfirmation;
};

const buildTuk = async (id) => {
    const tukConfirmation = await getTuk(id);
    ...
    console.log(tukConfirmation);
    const { receipt } = tukConfirmation;
    ...
    tuk.value = tukConfirmation.date;
    berhasil.checked = tukConfirmation.isProcessed;
};

const editTuk = async (id) => {
    const accessToken = localStorage.getItem('accessToken');
    ...
    submitBtn.addEventListener('click', async (event) => {
        ...
        const data = {
            date: document.getElementById('tuk').value,
        ...
        if (responseJson.status === 'success') {
            window.location.href = '#konfimrasipenerimaan';
        } else {
