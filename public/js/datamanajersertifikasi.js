...
const getManagerSertif = async () => {
    const accessToken = localStorage.getItem('accessToken');
    const response = await fetch('/api/cert-managers?isProcessed=true', {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${accessToken}`,
    ...
    const responseJson = await response.json();
    const certManagers = responseJson.data.certManagers;

    return certManagers;
};

const buildTable = () => {
    getManagerSertif().then((certManagers) => {
        const dataTable = $('#dataTable').DataTable();
        certManagers.forEach((certManager) => {
            const tukName =
                certManager.printBlank.printAssesion?.assTestResult?.baSk
            ...
            const managersertifProcessed = certManager?.isProcessed || false;
            ...
            const data = {
                ...
                managersertifProcessed:
                    managersertifProcessed === true
                        ? '<span style="color: green;">Berhasil</span>'
                        : '<span style="color: red;">Pending</span>',
                Action: `
                    <a
                        href="#inputmanajersertifikasi?id=${
                            certManager.id || ''
                        }"
        ...
        $('#dataTable tbody').on('click', 'tr', async function () {
            ...
            const id =
                certManagers[rowData]?.printBlank.printAssesion?.assTestResult
