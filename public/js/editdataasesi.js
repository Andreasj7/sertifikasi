...
const buildDataAsesiField = async (id) => {
    ...
    const tglDataAsesi = document.getElementById('data_asesi');
    ...
    noSptassesor.value = sptAssesor.assesorDate;
    tglSptassesor.value = sptAssesor.assesorDate;
    tglDataAsesi.value = dataAsesi.date;
    pengerjaan.checked = dataAsesi.isProcessed;
};
...
const editDataAsesi = async (id) => {
    ...
    submitBtn.addEventListener('click', async (event) => {
        ...
        if (responseJson.status === 'success') {
            window.location.href = '#datadataasesi';
        } else {
